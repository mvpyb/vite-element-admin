<template>
  <div v-loading="loading">
    <el-alert title="此处代码段警告参考，须配置文件下载请求" type="warning" style="margin: 30px;" />
    
    <el-input placeholder="请输入远程zip地址" v-model="url" clearable style="margin-bottom: 30px;"  />
    <el-button type="primary" @click="unzip" style="margin-bottom: 30px;" >解压文件</el-button>
...
<script setup>
  ...
  async function unzip() {
    ...
    loading.value = true
    try {
      const { zipReader, files, size } = await ReaderZip.readRemote( url )
  ...
  async function handleZip( zipReader, files ) {
    ...
    await Promise.all( keyData.map( async( item ) => {
